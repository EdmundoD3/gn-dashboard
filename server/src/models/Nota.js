...
const Nota = {
  post: async ({ cliente = {}, addres = {}, cuenta = {}, compra = [] }) => {
    ...
    // console.log(clienteDates, addresDates, cuentaDates, compraDates)
    const msErr = (!clienteDates ? "name is not found " : "") +
      (!addresDates ? "addres dates is not ok " : "") + (!cuentaDates ? "clienteID in cuenta is not ok " : "")
      + (!compraDates ? "compra is not ok" : "")
    if (!clienteDates || !cuentaDates || !addresDates || !compraDates) return { error: msErr }
...
const addresDate = ({ street = "", noAddress = "", betweenstreet = "", referencia = "",
  observation = "", stateId = "0", coloniaId = "0", cityId = "0", }) => {
  return {
...
const clienteDate = ({ name, lastName = '0', email = '0', phone = '0', date = '0', comments = '0', cobradorId, statusId, mongodbId = '0', vendedoraId }) => {
...
const compraDate = (compra = []) => {
  ...
  ({ productosId: productosId.toString(), cuentaID: cuentaId.toString(), cantidad: cantidad.toString() }))  
