const { addres: table } = require("../sql/tables");
...
const Addres = {
  ...
  getByStateId: async ({ stateId = 0 }) =>
    await getOne({ value: stateId, nameValue: "state_id" }),
  getByColoniaId: async ({ coloniaId = 0 }) =>
    await getOne({ value: coloniaId, nameValue: "colonia_id" }),
  getByCityId: async ({ cityId = 0 }) =>
    await getOne({ value: cityId, nameValue: "city_id" }),
  post: async ({
    clienteId = "",
    street = "",
    noAddress = "",
